...
!  M. Ristinmaa 2013-04-14
!   - Copied version from elem_large_mix_th_2d
!   - included an additional diffusion field for temperatur
!------------------------------------------------------------------------------	

!
!   Module elem_large_cont_2d_th contains element subroutines 
!   for 2-D mixture formulation (displacement - pressure - temperature)
!   The balance equations than can be solved are defined by the equation set
!
!    int(Bu^T s dA) - int(Nu^T t dS) =0
!    int(Bp^T hp dA) + int(Np^T gp dA) - int(Np^T qp dS)=0
!    int(Bt^T ht dA) + int(Nt^T gt dA) - int(Nt^T qt dS)=0
!
!   where the possible constitutive assumptions are
!   UPDATE DEPENDENCIES
!
!    ht=ht(grad(u),p,grad(p))
!    gt=gt(grad(u),p)
!    q=constant
!    s=s(grad(u),p)
!    t=constant
!
!   - Q2Q1 2D Taylor-Hood element 
!     (8 node for displacements, 4 for pressure, 4 for temperature)
!  
!   nodal numbering: corner node Counter Clock Wise, then mid node CCW
!   number of 
!   dof=(4,4,4,4,2,2,2,2) total 24 dofs
!   4 for corner nodes (displacements, pressure and temperature)
...
integer, parameter,dimension(16):: u_list=(/ 1, 2, & 
                                             ...
                                             21, 22, &
                                             23, 24/)
integer, parameter,dimension(4) :: p_list=(/3,7,11,15/)
integer, parameter,dimension(4) :: t_list=(/4,8,12,16/)
...
subroutine m2dpulq2q1_d(dg,pg,gradp,tg,gradt,coord,ed)
  implicit none

  double precision                :: dg(:,:), pg(:), gradp(:,:)
...
! current nodal coordinates
  ucoord(1,:)=coord(1,:)+ed([1,5,9,13,17,19,21,23])
  ucoord(2,:)=coord(2,:)+ed([2,6,10,14,18,20,22,24])
...
! pressure and pressure gradient in gauss points
    pg(igp)=dot_product(Np(igp,:),ed(p_list))
    gradp(:,igp)=matmul(dNpx,ed(p_list))

! temperature and temperature gradient in gauss points
    tg(igp)=dot_product(Np(igp,:),ed(t_list))
    gradt(:,igp)=matmul(dNpx,ed(t_list))
...
subroutine m2dpulq2q1_f(fe,coord,ed,es,hp,gp,ht,gt)
  implicit none

  double precision                :: fe(:), coord(:,:),  ed(:)
  double precision                :: es(:,:), hp(:,:), gp(:)
  double precision                :: ht(:,:), gt(:)

  double precision                :: JT(nrgpd,2), JTinv(2,2), detJ
  double precision                :: dNux(2,8), dNpx(2,4), Bu(3,16)

  double precision                :: efu(16), efp(4), eft(4)
...
! strain displacement matrix
    ...
    efp=efp+(matmul(transpose(dNpx),hp(:,igp)) &
             +Np(igp,:)*gp(igp))*detJ*w_1(igp)*w_2(igp)
    eft=eft+(matmul(transpose(dNpx),ht(:,igp)) &
             +Np(igp,:)*gt(igp))*detJ*w_1(igp)*w_2(igp)
...
end module elem_large_mix_th_2dt
