...
subroutine eq_geneigenvalue3_sparse_bcnx(K,Mass,ndof,BCdof,BCval,dofnod,Emin,Emax,L,sfactor,phi,nbrw,info)
  implicit none
  type(sparse)                    :: K, Mass
  double precision                :: BCval(:), Emin, Emax
  integer                         :: bcdof(:)
  ...
  integer, allocatable            :: INX(:)
  integer                         :: i, status, p, node, nent
  double precision, allocatable    :: E(:), X(:,:), res(:), phi(:,:), sfactor(:)
  integer                         :: M0, M, L, info
  ...
  allocate(E(ndof),stat=status)
  allocate(X(ndof,M0),stat=status)
  allocate(phi(nbrw,ndof),stat=status)
  allocate(sfactor(nbrw),stat=status)
! Note that this routine does not allow that 
! bc contains redundant constraints
...
! would be nice to get rid of the allocation part
  ...
  do i=1,nent
    ...
    ! If bc is described here
      ...
      do p=K%ia(i),K%ia(i+1)-1
        ! Which column
        j=K%ja(p)
        ! If we are on the diagonal, leave the entry
        if (i.eq.j) then
        ! Else zero
        else
          K%a(p)=0d0
  ...
  call eq_geneigenvalue3_sparse(K,Mass,ndof,M,X,Emin,Emax,L,sfactor,phi,nbrw,info)
...
subroutine eq_geneigenvalue3_sparse(K,Mass,ndof,M,X,Emin,Emax,L,sfactor,phi,nbrw,info)
  implicit none
  type(sparse)                     :: K, Mass


  integer                          :: fpm(128), info, nbrw, ii
  character*1 UPLO
  parameter   (UPLO='F')
  double precision                 ::  epsout, Emin,Emax, sfactor(:), phi(:,:)
  integer     loop
  integer     L
  integer     			   			  :: M0,M, ndof, status
  double precision, allocatable    :: E(:), X(:,:), res(:)
  ...
  ! M returns number of found eigenvalues in [Emin, Emax]
  call dfeast_scsrgv(UPLO,ndof,K%a, K%ia,K%ja,Mass%a, Mass%ia,Mass%ja,fpm,epsout,loop,Emin,Emax,M0,E,X,M,res,info)
  !sfactor=E(1:3)
  !nbrw = M
  !phi = 0d0
...
subroutine adjoint(hp,edrho,edphis,edphik,ed,coord,enod,removed,delta0,B,eta,p,Hprojection,dofnod,pdeFilter,rhotilde)
  ...
  double precision                 :: CRAP98(9,8), Ones(8), GradFATP(6,8), Dgp(6,6,8), Bl(6,8), farg(6,8), Aarg(6,8), Targ(6,8), g, hep(24), tmp24(24), Etmps(6,8), Etmpk(6,8), Etmp2(6,8), Etmp3(6,8)
  ...
  do ie=1,nelm	
		...
		! s = v => T, k = phi => A
		! E(a,v) = T
      call c3dtl8_emu(coord(:,enod(:,ie)),ed(:,ie),edphis(:,ie),Etmps,crap98,Ones) 
		
		! E(a,phi) = A
      call c3dtl8_emu(coord(:,enod(:,ie)),ed(:,ie),edphik(:,ie),Etmpk,crap98,Ones) 
      ...
      ! The deformation gradient 
		...
		call dfat(GradFATP,dg(:,:,ie),Etmpk,Etmps) 
      ...
      do igp=1,ngp
            ...
            farg(:,igp)=(matmul(Dgp(:,:,igp),Bl(:,igp))+1D0*GradFATP(:,igp)*g) ! Note that 2D0 is replaced by 1D0 since the factor 2D0 is inside sub. dfat
            Targ(:,igp)=matmul(Dgp(:,:,igp),Etmps(:,igp))
            Aarg(:,igp)=matmul(Dgp(:,:,igp),Etmpk(:,igp))
			else
				Dgp(:,:,igp) = 0d0

            farg(:,igp)=(matmul(Dgp(:,:,igp),Bl(:,igp))+1D0*GradFATP(:,igp)*0d0)
            Targ(:,igp)=matmul(Dgp(:,:,igp),Etmps(:,igp))
            Aarg(:,igp)=matmul(Dgp(:,:,igp),Etmpk(:,igp))
			endif 
      enddo
		...
		! First term in h
		call c3dtl8_f(hep,coord(:,enod(:,ie)),ed(:,ie),farg)

		! The second term is split in two and then added to hep
      call c3dtl8_f(tmp24,coord(:,enod(:,ie)),edphik(:,ie),Targ) 
      hep=hep+tmp24
      call c3dtl8_f(tmp24,coord(:,enod(:,ie)),0D0*edphik(:,ie),Targ) 
      hep=hep-tmp24

		! As well as the third term
		call c3dtl8_f(tmp24,coord(:,enod(:,ie)),edphis(:,ie),Aarg) 
      hep=hep+tmp24
      call c3dtl8_f(tmp24,coord(:,enod(:,ie)),0D0*edphis(:,ie),Aarg) 
      hep=hep-tmp24
...
subroutine fsk(ffilter,omega,edphis,edphik,edrho,ed,edmu,coord,enod,removed,delta0,B,eta,p,Hprojection,massRho,pdeFilter,rhotilde,KM,adj)
  implicit none

  double precision                 :: edphis(:,:), edphik(:,:), edrho(:,:), edmu(:,:), rhogp(8), ffilter(:), rhotilde(:), KM(:), adj(:)
  double precision					  :: ed(:,:), coord(:,:), valm, valf
  double precision, allocatable	  :: dg(:,:,:), es(:,:,:)
  integer								  :: removed(:), enod(:,:)
  double precision                 :: CRAP98(9,8), Ones(8), Dgp(6,6,8), AargP(6,8), gp, esscaled(6,8), tmp6(6), fen(8), fme(8), Etmps(6,8), Etmpk(6,8), Etmp2(6,8), Etmp3(6,8), Etmp5(6,8), Scalar(8), scalarKM(8), scalaradj(8), fenadj(8), fenKM(8)
  ...
  do ie=1,nelm
      ...
      do igp=1,ngp
			 ...
			 Scalaradj(igp) = dot_product(Etmp5(:,igp),esscaled(:,igp))
			 
			 ! First term in K
			 tmp6=matmul(Dgp(:,:,igp),Etmpk(:,igp))
			 phiKphi=dot_product(Etmps(:,igp),tmp6)

		    ! Second term
			 tmp6=Etmp3(:,igp)-Etmp2(:,igp) 
			 phiKphi=phiKphi+dot_product(tmp6,esscaled(:,igp))

			 scalarKM(igp) = phiKphi
      ...
      enddo
		...
		call fl3d8_bf(fenadj,coord(:,enod(:,ie)),Scalaradj)
		
		call fl3d8_bf(fenKM,coord(:,enod(:,ie)),ScalarKM)
		...
		if (pdeFilter) then
         ...
         KM(ie) = dot_product(fenKM,ones) - omega*valm
         
         adj(ie) = dot_product(fenadj,ones) 
...
subroutine fskInter(ffilter,omega,edphis,edphik,edrho,ed,edmu,coord,enod,removed,delta0,B,eta,p,Hprojection,massRho,c1,c2,pdeFilter,rhotilde)
  implicit none
