module mater_hyperel_elect_visco_stages
...
! M. Ristinmaa 2012-01-06
!  - several stages (states) are stored and can be used
!  - old and new are replaced with states
! ------------------------------------------------------------------------------

! ToDo
! move vis_update into stress call ??
! -this split might be good since the stresses can
! be calculated without very much cost.
...
double precision, allocatable     :: Cv_init(:,:)
private mpK, mpMy, mpCm, mpCe, mpEo, nvis, mpBvis, mpGvis, intVar, Cv_init
...
subroutine neohooke_vis_elect_init(mp, nstage, nelem, nr_gp)
  implicit none
  double precision                :: mp(:)
  integer                         :: nstage, nelem, nr_gp, ierr, ie, iv, igp, ist
  mpK =mp(1)
  mpMy=mp(2)
  mpCm=mp(3)
  mpCe=mp(4)
...
!  mpEo=mp(5)
  ...
  allocate(intVar(nr_gp,nvis,6,nelem,nstage), stat=ierr)
  allocate(Cv_init(nvis,6), stat=ierr)
  ! initiate internal variables
  do iv=1,nvis
      Cv_init(iv,:)=(/1d0, 1d0, 1d0, 0d0, 0d0, 0d0/)
  end do

  do ie = 1,nelem
    do igp = 1,nr_gp
      do ist=1,nstage
        intVar(igp, :, :, ie, ist) = Cv_init
      end do
    end do
  end do
...
subroutine neohooke1(stype,stress,edisp,ef,de,ie,ist)
  implicit none
  double precision                :: stress(:,:), edisp(:,:), ef(:,:), de(:,:)
  character(len=*)                :: stype
  integer                         :: ie, gp, nr_gp, ist

  nr_gp=size(ef,2)

  do gp=1,nr_gp
    call neohooke2(stype,stress(:,gp), intVar(gp,:,:,ie,ist), edisp(:,gp),ef(:,gp), de(:,gp))
  enddo
...
subroutine dneohooke1(stype,Duu,Dup,Dpu,Dpp,ef,e,ie,ist,dt)
  implicit none
  ...
  do gp=1,nr_gp
    call dneohooke2(stype,Duu(:,:,gp),Dup(:,:,gp),Dpu(:,:,gp),Dpp(:,:,gp), &
                    ef(:,gp),e(:,gp),intVar(gp,:,:,ie,ist),dt)
  enddo
...
subroutine update1(stype,ef, dt, ie, ist)
  implicit none
  double precision      :: ef(:,:), dt, Cv_gp(nvis,6)
  character(len=*)                :: stype
  integer               :: ie, nr_gp, gp, ist
  
  nr_gp = size(ef,2)
  if(nvis>0) then
    do gp=1,nr_gp
! SHOULD NOT THE INTEGRATION BE FROM STATE IST-1 TO STATE IST
! INTEGRATION FROM ACCEPTED STATE IST-1
! NOTE SPECIAL CARE MUST BE TAKEN WHEN IST=1
!     call update2(stype,ef(:,gp), dt, intVar(gp,:,:,ie,old,ist), Cv_gp)
      if (ist.eq.1) then
        call update2(stype,ef(:,gp), dt, Cv_init, Cv_gp)
      else  
        call update2(stype,ef(:,gp), dt, intVar(gp,:,:,ie,ist-1), Cv_gp)
      end if
      intVar(gp,:,:,ie,ist) = Cv_gp
    end do 
  end if
  return
...
subroutine state_accept(ist)
  implicit none
  integer          :: ist

! when we store the stages this does not make sence as the integration
! is always taken from the last state to the current
! 

! This copy must be done otherwise the tangent stiffness
! in the first iteration is calculated at the n-1 state
!  intVar(:,:,:,:,Old,ist)=intVar(:,:,:,:,New,ist)
...
! Copy the state
  intVar(:,:,:,:,ist)=intVar(:,:,:,:,istn)

  return
end subroutine state_copy

subroutine reset_state(ist)
  implicit none
  integer          :: ist, ie, igp, nelem, nr_gp
...
! don't declear nvis global
    integer                         :: nr_gp, nelem, nstate, ierr
...
!  INQUIRE(IOLENGTH=length) specimen, list, of, items. Require DIRECT access
  ...
  nr_gp =size(intVar,1)
  nelem =size(intVar,4)
  nstate=size(intVar,5)

  write(1,*) nr_gp, nvis, nelem, nstate
  write(1,*) mpK, mpMy, mpCm, mpCe, mpEo
  write(1,*) mpBvis
  write(1,*) mpGvis
  write(1,*) intVar
  write(1,*) Cv_init
...
!  INQUIRE(IOLENGTH=length) specimen, list, of, items 
  ...
  read(1,*) nr_gp, nvis, nelem, nstate
  read(1,*) mpK, mpMy, mpCm, mpCe, mpEo

!  write(*,*), nr_gp, nvis,nelem, nstate
! Allocate structures
  allocate(mpBvis(nvis), stat=ierr)
  allocate(mpGvis(nvis), stat=ierr)
  allocate(intVar(nr_gp,nvis,6,nelem,nstate), stat=ierr)
  allocate(Cv_init(nvis,6), stat=ierr)
...
function state_numbers()
  ...
  state_numbers=size(intVar,5)

  return
end function state_numbers


subroutine getIntVar1(ist, ie, Cv)
...
subroutine getIntVar2(ist, ie, Cv)
...
subroutine dineohooke1(stype,Duu,Dup,Dpu,Dpp,es,d,ef,e,ie,ist,dt)
  implicit none
  double precision                :: Duu(:,:,:), Dup(:,:,:)
  double precision                :: Dpu(:,:,:), Dpp(:,:,:)
  ...
  do gp=1,nr_gp
    call dineohooke2(stype,Duu(:,:,gp),Dup(:,:,gp),Dpu(:,:,gp),Dpp(:,:,gp), &
                    es(:,gp),d(:,gp),ef(:,gp),e(:,gp),ie,dt,gp,ist)
  enddo
...
subroutine dineohooke2(stype,Duu,Dup,Dpu,Dpp,es,dd,ef,e,ie,dt,gp,ist)
  implicit none
  double precision                :: Duu(:,:), Dup(:,:), Dpu(:,:), Dpp(:,:)
...
!    call neohooke_elect('Cauchy-inv',esx2,ddx,ef,ex,th) 
    call neohooke2('Cauchy-inv',esx2,intVar(gp,:,:,ie,ist),ddx,ef,ex)
...
!  write(*,*)'dpu_ana ',dpu
  ...
  do ii=1,9
    efx=ef
    efx(ii)=efx(ii)+pert    
!    call neohooke_vis_elect('Cauchy-inv',esx2,ddx, &
!                              dg(:,:,ie),efx,ie,ist)
!
!    call neohooke_elect('Cauchy-inv',esx2,ddx,efx,e,th) 
!    call update2('Cauchy-inv',efx, dt, intVar(gp,:,:,ie,old,ist), Cv_gp)
!    call neohooke2('Cauchy-inv',esx2,Cv_gp,ddx,efx,e)
    dpu(:,ii)=(ddx-dd)/pert
  enddo
!  write(*,*)'dpu_per ',dpu
...
if (1.eq.0) then
  ...
  do ii=1,3
    ...
    ex(ii)=ex(ii)+pert    
!    call neohooke_elect('Cauchy-inv',esx2,ddx,ef,ex,th) 
!    call neohooke2('Cauchy-inv',esx2,intVar(gp,:,:,ie,new,ist),ddx,ef,ex)
    call neohooke2('Cauchy-inv',esx2,intVar(gp,:,:,ie,ist),ddx,ef,ex)
    dpp(:,ii)=(ddx-dd)/pert
  enddo
  write(*,*)'dpp_per ',dpp
...
end module mater_hyperel_elect_visco_stages
