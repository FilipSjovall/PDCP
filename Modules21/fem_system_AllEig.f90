...
subroutine eq_geneigenvalue3_sparse_bcnx(K,Mass,ndof,BCdof,BCval,dofnod,Emin,Emax,L,sfactor,phi,nbrw,info,allEig)
  implicit none
  type(sparse)                    :: K, Mass
  double precision                :: BCval(:), Emin, Emax
  integer                         :: bcdof(:)
  ...
  integer, allocatable            :: INX(:)
  integer                         :: i, status, p, node, nent
  double precision, allocatable    :: E(:), X(:,:), res(:), phi(:,:), sfactor(:), allEig(:)
  integer                         :: M0, M, L, info
...
! would be nice to get rid of the allocation part
  ...
  call eq_geneigenvalue3_sparse(K,Mass,ndof,M,X,Emin,Emax,L,sfactor,phi,nbrw,info,allEig)
...
subroutine eq_geneigenvalue3_sparse(K,Mass,ndof,M,X,Emin,Emax,L,sfactor,phi,nbrw,info,allEig)
  implicit none
  type(sparse)                     :: K, Mass
  ...
  integer     L
  integer     			   			  :: M0,M, ndof, status
  double precision, allocatable    :: E(:), X(:,:), res(:),allEig(:)
  ...
  if (allocated(allEig)) then
  		deallocate(allEig)
  end
  
  allocate(allEig(M),stat=status)
