...
subroutine adapMesh(rmv,enod,coord,a,fext,bcnode,bcval,adaplevel)
  implicit none
...
! nodes can be stored
  ...
  call reallocate(coord,size(coord,1),max_nr_nodes)
...
! allocate extra space such that new elements can be stored
  ...
  call reallocate(enod,size(enod,1),max_nr_elements)
...
! external boundary
  ...
  call neighbel(lele,nside,enod,iel)
...
! nodes defining element
  ...
  ele_min=ele
  ex=(/coord(1,enod(1,ele_min)),coord(1,enod(2,ele_min)),coord(1,enod(3,ele_min)) /) 
  ey=(/coord(2,enod(1,ele_min)),coord(2,enod(2,ele_min)),coord(2,enod(3,ele_min)) /) 
  DetA=ex(2)*ey(3)+ex(1)*ey(2)+ey(1)*ex(3) &
         -ex(2)*ey(1)-ex(3)*ey(2)-ey(3)*ex(1)
  if (DetA.le.0d0) then
    write(*,*)' Area negative'
    write(*,*)'enod ',enod(:,ele_min)
    write(*,*)'coord1 ',coord(1:2,enod(1,ele_min))
    write(*,*)'coord2 ',coord(1:2,enod(2,ele_min))
    write(*,*)'coord3 ',coord(1:2,enod(3,ele_min))
    write(*,*)'n ',n
    do i1=1,3
      write(*,*)'n ',n(i1),' coord ',coord(1:2,n(i1))
    enddo
    write(*,*)'maxnod ',maxnod,' coord ',coord(1:2,maxnod)
!    enod(:,ele)=(/maxnod, n(side), n(side+2) /)
  ...
  if (DetA.le.0d0) then
    write(*,*)' Area negative'
    write(*,*)'enod ',enod(:,ele_min)
    write(*,*)'coord1 ',coord(:,enod(1,ele_min))
    write(*,*)'coord2 ',coord(:,enod(2,ele_min))
    write(*,*)'coord3 ',coord(:,enod(3,ele_min))
    write(*,*)'n ',n
    do i1=1,3
      write(*,*)'n ',n(i1),' coord ',coord(1:2,n(i1))
    enddo
...
! coordinates for nodes in the element
  ...
  ex=(/coord(1,n(1)), coord(1,n(2)), coord(1,n(3))/)
  ey=(/coord(2,n(1)), coord(2,n(2)), coord(2,n(3))/)
...
! split boundary connecting the elements on the longest element side 
  ...
  call neighbel(lele,dummy,enod,iel)
...
write(*,*)'6'
  ...
  do ie=1,current_nr_elements
    enod(1,ie)=nodmap(enod(1,ie))
    enod(2,ie)=nodmap(enod(2,ie))
    enod(3,ie)=nodmap(enod(3,ie))
  end do
!
...
! we know that a node exist between two other nodes
  ...
  do i1=1,3
    do i2=1,3
      if (enod(i1,ele_max).eq.enod(i2,ele_min)) then
        i3=i3+1
        cnod(i3)=enod(i1,ele_max)
        pos_max(i3)=i1
        pos_min(i3)=i2
      endif
    enddo
  ...
  enod(pos_max,ele_max)=-enod(pos_max,ele_max)
  n1=maxval(enod(:,ele_max))
  enod(pos_max,ele_max)=-enod(pos_max,ele_max)

  enod(pos_min,ele_min)=-enod(pos_min,ele_min)
  n2=maxval(enod(:,ele_min))
  enod(pos_min,ele_min)=-enod(pos_min,ele_min)

  xm=(coord(1,n1)+coord(1,n2))*0.5d0
  ym=(coord(2,n1)+coord(2,n2))*0.5d0

  d1=(xm-coord(1,cnod(1)))**2d0+(ym-coord(2,cnod(1)))**2d0
  d2=(xm-coord(1,cnod(2)))**2d0+(ym-coord(2,cnod(2)))**2d0

  if (d1.lt.d2) then
! cnod(1) removed   
    enod(pos_min(1),ele_min)=n1
  else
! cnod(2) removed   
    enod(pos_min(2),ele_min)=n1
  endif
